...
/*
 * Training Videos — the video tab of LMS ▸ Content.
 *
 * Was its own page at /lms/training-videos until 2026-08-26. Video is now one
 * of three CONTENT KINDS a course can hold (video / document / assessment), so
 * the three catalogues are tabs of one page rather than three sidebar leaves —
 * the sidebar is a hard two-level tree (Sidebar.tsx::buildTree re-parents
 * grandchildren), and an operator building a course moves between the kinds
 * constantly. Behaviour here is UNCHANGED from that page; only the surrounding
 * header moved out to the tab shell.
 *
 ...
 *
 * Scope note: this tab manages the CATALOGUE ENTRY (title / description),
 ...
 * there is no upload control here. Documents, on the neighbouring tab, DO
 * carry their file — they are an EasyFix-owned table with an S3 key.
 */

import { useEffect, useState } from 'react';
import { Search, Plus, Pencil, Trash2, AlertTriangle, Play, Youtube, FileVideo } from 'lucide-react';
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          {/* Sets expectations before an operator hunts for an upload button
              that does not exist on this tab — Documents next door DOES have
              one, which makes the difference worth stating here. */}
          <p className="text-xs text-muted-foreground mt-1 max-w-3xl">
            The playable video file itself is managed outside the CRM — it resolves through the
            legacy document store via <span className="font-mono">training_video_id</span>. This tab
