...
export function lifecycleTargets(snapshot: EasyfixerLifecycleSnapshot): EasyfixerLifecycleStatus[] {
  // Blacklist is no longer terminal: Ops can always move a technician out of it
  // (INACTIVE / DORMANT / re-affirm BLACKLISTED). The backend now sends real
  // `allowedTransitions` for a blacklisted technician, so we no longer hard-stop
  // here — the provided graph (or the operational fallback below) drives the list.
