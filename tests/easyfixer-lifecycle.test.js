...
test('blacklisted is reversible and history aliases are normalized', () => {
  // Blacklist is an admin decision, so Ops can fully reverse it — the
  // conservative fallback (no server allowedTransitions) now surfaces the whole
  // operational set instead of an empty, terminal list.
  const blacklisted = lifecycle.normalizeLifecycleSnapshot({ status: 'BLACKLISTED' });
  assert.ok(blacklisted);
  const blacklistTargets = lifecycle.lifecycleTargets(blacklisted);
  assert.ok(blacklistTargets.includes('ACTIVE'), 'Blacklisted must be reversible to Active');
  assert.equal(blacklistTargets.includes('BLACKLISTED'), false, 'never lists itself');
  assert.equal(blacklistTargets.includes('NEW'), false, 'cannot fall back into onboarding');
